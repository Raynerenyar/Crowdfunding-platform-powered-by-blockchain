...
export class PrimeMessageService implements OnDestroy {
  ...
  detailedInfoMethod(summary: string, detail: string) {
    this.primeMsgSvc.add({
      severity: 'info', summary: summary, detail: detail, ...this.commonProperty
    })
  }
  ...
  generalInfoMethod(msg: string) {
    this.primeMsgSvc.add({
      severity: 'info', summary: "Info", detail: msg, ...this.commonProperty
    })
  }

  detailedWarnMethod(summary: string, msg: string) {
    this.primeMsgSvc.add({
      severity: 'warn', summary: summary, detail: msg, ...this.commonProperty
    })
  }

  tellToConnectToChain() {
    this.primeMsgSvc.add({ severity: 'error', summary: 'Wrong Network', detail: 'Please connect to the correct network' })
  }
