...
import ethereum.tutorials.java.ethereum.util.Util;
...
public class AuthController {
    ...
    @Autowired
    private BlockchainEventListener bListener;
    ...
    @GetMapping(path = "/contribute")
    public ResponseEntity<String> contractContribute(@RequestParam String address) {
        System.out.println("contribute api address received >>>>>> " + address);
        System.out.println(crowdfundingAbi);
        String forResponse = BcSvc.testRun(address);
        System.out.println("resp >>>> " + forResponse);
        JsonObject jsonObj = Json.createObjectBuilder().add("encodedTransaction", forResponse).build();
        return ResponseEntity.status(HttpStatus.OK).body(jsonObj.toString());
    }

    @GetMapping(path = "/abi")
    public ResponseEntity<String> getContractAbi() {
        JsonObject jsonObj = Json.createObjectBuilder()
                .add("abi", crowdfundingAbi)
    ...
    @GetMapping(path = "/contributes")
    public ResponseEntity<String> contractFunction(@RequestParam String functionName, @RequestParam Object params) {
        try {
            Optional<String> opt = BcSvc.xxgetFunctionEncoded(functionName, contractAddress, params);
            System.out.println("testing s>> " + opt.get());
        } catch (Exception e) {
            e.printStackTrace();
        }
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
